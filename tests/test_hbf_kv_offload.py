...
class HBFKVOffloadTest(unittest.TestCase):
    ...
    def test_hbf_kv_need_size_and_avail_size(self):
        ...
        # The new block-pool architecture exposes availability as the HBF KV
        # ledger remaining: is_avail reflects it directly, and there is no
        # separate need_size accumulator.
        self.assertTrue(memory.is_avail(capacity, Device.HBF))
        self.assertFalse(memory.is_avail(capacity + 1, Device.HBF))
...
class HBFPrefixStorageTest(unittest.TestCase):
    def test_hbf_prefix_storage_uses_per_rank_page_size(self):
        ...
        # The new architecture models the second-tier cache as a BlockPool
        # (self.storage_pool). HBF is per-GPU, so it keys on the NPU block_size
        # rather than the CPU/CXL full-cluster chunk.
        self.assertIsNotNone(memory.storage_pool)
        self.assertEqual(memory.storage_pool.tier, Device.HBF)
        self.assertEqual(memory.storage_pool.block_size, 16)
