...
class HBFWeightResidencyTest(unittest.TestCase):
    def test_all_hbf_weights_release_hbm_for_kv(self):
        ...
        # The new block-pool architecture sizes KV capacity from npu_mem minus
        # the HBM-resident weight; with all weights on HBF the whole NPU is KV.
        self.assertEqual(
            memory.npu_pool.bytes_per_block * memory.npu_pool.num_blocks,
            GB_TO_BYTE,
        )
        self.assertEqual(
    ...
    def test_absent_hbf_keeps_legacy_weight_accounting(self):
        ...
        # KV capacity is (npu_mem * util - hbm_weight) rounded down to whole
        # blocks; the pool never exposes a partial block of KV.
        kv_bytes = int(memory.npu_mem * memory.npu_memory_utilization) - memory.hbm_weight
        self.assertEqual(
            memory.npu_pool.bytes_per_block * memory.npu_pool.num_blocks,
            (kv_bytes // memory.npu_pool.bytes_per_block) * memory.npu_pool.bytes_per_block,
        )
