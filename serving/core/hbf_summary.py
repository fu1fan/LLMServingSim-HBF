...
def build_hbf_runtime_summary(instances, schedulers, num_devices, run_id):
    memory_rows = []
    for instance_id, (instance, scheduler) in enumerate(
        zip(instances, schedulers)
    ):
        ...
        memory_rows.append(
            {
                ...
                "hbm_kv_capacity_bytes": memory.npu_mem
                - memory.hbm_weight,
                "hbf_capacity_bytes": (
                    memory.hbf_memory.capacity_bytes
                    if memory.hbf_memory is not None
                    else 0
                ),
    ...
    return {
        ...
        "hbf_energy_unmodeled": any(
            instance.get("hbf_mem") is not None for instance in instances
        ),
