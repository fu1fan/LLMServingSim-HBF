...
MODEL_CONFIG="$REPO_ROOT/configs/model/meta-llama/Llama-3.1-405B-Instruct.json"
OFFICIAL_VERIFICATION="$REPO_ROOT/models/Llama-3.1-405B-Instruct/OFFICIAL_VERIFICATION.txt"
HARDWARE="B200"
TP_DEGREES="1,2,4,8"
...
if [[ ! -s "$OFFICIAL_VERIFICATION" ]]; then
  echo "Official Meta config verification is required before GPU use." >&2
  echo "Run: HF_TOKEN=... ./scripts/verify_llama405b_metadata.sh" >&2
  exit 2
fi
...
log_file="$LOG_ROOT/llama405b-${timestamp}.log"
...
profile_dir="$PROFILE_OUT_ROOT/B200/meta-llama/Llama-3.1-405B-Instruct/bf16"
